...
class CallChainBuilder:
    ...
    def get_detailed_call_chain(
        ...
        if self.debug:
            debug(f"[CallChainBuilder] Building call chain from line {source_line} to {sink_line}")
    ...
    def _deduplicate_and_sort(self, call_chain: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
