...
class TaintPath:
    ...
    def __post_init__(self):
        self.path_length = len(self.intermediate_nodes) + 2  # source + sink + intermediates
...
class CallChainTracker:
    ...
    def enter_function(self, func_name: str, line: int, col: int, parameters: List[str] = None):
    ...
    def exit_function(self, func_name: str, return_node: Optional[CallChainNode] = None):
    ...
    def track_source(self, node: ast.Call, source_info: Dict[str, Any]) -> CallChainNode:
        ...
        if self.debug:
            logger.debug(f"[CallChain] Tracked source: {source_node.function_name} at line {source_node.line_number}")
    ...
    def track_function_call(self, func_name: str, line: int, col: int, 
                           arguments: List[str] = None) -> CallChainNode:
        ...
        if self.debug:
            logger.debug(f"[CallChain] Tracked function call: {func_name} at line {line}")
        
        return call_node
    
    def create_taint_path(self, source_node: CallChainNode, sink_node: CallChainNode,
                         tainted_var: str) -> TaintPath:
        ...
        if tainted_var in self.variable_assignments:
            for assignment in self.variable_assignments[tainted_var]:
                if (source_node.line_number < assignment.line_number < sink_node.line_number):
        ...
        if self.debug:
            logger.debug(f"[CallChain] Created taint path: {source_node.function_name} -> {sink_node.function_name} (confidence: {confidence:.2f})")
    ...
    def get_detailed_paths(self) -> List[Dict[str, Any]]:
        ...
        for path in self.taint_paths:
            path_info = {
                ...
                "complexity": "low" if path.path_length <= 3 else "medium" if path.path_length <= 6 else "high"
            }
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_path_length": sum(p.path_length for p in self.taint_paths) / len(self.taint_paths) if self.taint_paths else 0,
            "high_confidence_paths": len([p for p in self.taint_paths if p.confidence > 0.8]),
