...
class StandardLibraryDetector:
    ...
    def stdlib_modules(self) -> Set[str]:
        ...
        if hasattr(sys, 'stdlib_module_names'):
            ...
            if self.debug:
                debug(f"Found {len(sys.stdlib_module_names)} modules via sys.stdlib_module_names")
        else:
            # Fallback for older Python versions
            stdlib_set.update(self._get_fallback_stdlib_modules())
            if self.debug:
                debug(f"Using fallback stdlib detection, found {len(stdlib_set)} modules")
        ...
        try:
            ...
            if spec:
                ...
                if spec.origin:
                    ...
                    elif "lib/python" in spec.origin and "site-packages" not in spec.origin:
    ...
    def _get_fallback_stdlib_modules(self) -> Set[str]:
        ...
        return {
            '__future__', '_abc', '_ast', '_asyncio', '_bisect', '_blake2', '_bootsubprocess',
            ...
            '_elementtree', '_functools', '_gdbm', '_hashlib', '_heapq', '_imp', '_io', '_json',
            '_locale', '_lsprof', '_lzma', '_md5', '_multibytecodec', '_multiprocessing', '_opcode',
            '_operator', '_pickle', '_posixsubprocess', '_py_abc', '_pydecimal', '_pyio', '_queue',
            '_random', '_sha1', '_sha256', '_sha3', '_sha512', '_signal', '_socket', '_sqlite3',
            '_sre', '_ssl', '_stat', '_statistics', '_string', '_strptime', '_struct', '_symtable',
            '_thread', '_threading_local', '_tkinter', '_tokenize', '_tracemalloc', '_uuid',
            '_warnings', '_weakref', '_weakrefset', '_zoneinfo', 'abc', 'aifc', 'argparse',
            'array', 'ast', 'asynchat', 'asyncio', 'asyncore', 'atexit', 'audioop', 'base64',
            'bdb', 'binascii', 'binhex', 'bisect', 'builtins', 'bz2', 'calendar', 'cgi',
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detection_method": "sys.stdlib_module_names" if hasattr(sys, 'stdlib_module_names') else "fallback",
        }
